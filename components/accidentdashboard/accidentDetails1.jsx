...
// --- Sub-Component for the Header (Status and Title) ---
const AccidentHeader = ({ status, title, statusColors }) => (
...
// --- A More Intelligent Action Button Component ---
// This component now handles the logic and appearance for all status changes.
const ActionButton = ({ id, currentStatus, onUpdate }) => {
    const [isLoading, setIsLoading] = useState(false);

    // A single, reusable function to update the status in Firebase
    ...
    // Conditionally render the correct button based on the current status
    ...
    // If status is "DONE" or anything else, show a disabled "Attended" button
    return (
        <button 
            disabled={true}
    ...
    <div className="w-full mb-2 text-left">
      <div className="flex flex-col w-full h-full gap-4 border-l-4 border-red-500 rounded-lg p-4 bg-gray-800/80 hover:bg-gray-800 transition-colors shadow-md">
        <div className="flex w-full items-start justify-between">
          ...
          <AccidentHeader status={status} title={title} statusColors={statusColors} />
        ...
        {/* We now render the dynamic ActionButton component at the bottom */}
