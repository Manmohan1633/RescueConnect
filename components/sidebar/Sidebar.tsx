...
// Data for our navigation links - easy to add or remove items
const navLinks = [
  {
    href: "/dashboard",
    label: "Dashboard",
  ...
  {
    href: "/profile", // Changed from /list to avoid duplication
    label: "Profile",
    icon: (
      <svg
        className=" w-7 h-7 stroke-current"
...
const SidebarLink = ({ href, label, icon }) => {
  ...
  const baseClasses =
    "flex items-center justify-center w-full h-12 py-9 mt-2 rounded-l-xl transition-colors duration-200";
  const activeClasses =
    "bg-rose-100 text-red-600 border-l-4 border-red-600";
  const inactiveClasses =
    "text-gray-500 hover:bg-rose-100 hover:text-red-600";
...
export default function Sidebaruser() {
  return (
    <div className="flex w-full min-h-10 flex-col items-center gap-10 rounded-2xl bg-red-50 p-0 sm:h-full">
      {/* Navigation Links */}
      <nav className="hidden w-full flex-col items-center sm:flex">
        {navLinks.map((link) => (
          <SidebarLink
            key={link.href}
            href={link.href}
      ...
      {/* Emergency Button */}
      {/* CHANGE: Removed "mt-auto" and added a smaller margin "mt-4" */}
      <div className="mt-4 mb-4">
        <Link href="/distressForm" className="block">
          <Image
            src="/images/emerge.png"
            alt="Emergency Distress Button"
            width={80}
            height={80}
