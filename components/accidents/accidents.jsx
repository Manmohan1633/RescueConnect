...
export default function Accidents({ onUpdate }) { // Accepts onUpdate to refresh
  const { accidents, loading, error, refreshAccidents } = useAccidentsData();
  const [filter, setFilter] = useState("All");

  // --- THIS IS THE FIX ---
  // This hook now correctly counts statuses from your data (e.g., "NEW", "PENDING", "DONE")
  const statusCounts = useMemo(() => {
    return accidents.reduce(
      (counts, acc) => {
        // Use "NEW" as a fallback if status is missing or empty
      ...
      // Initialize with the exact, uppercase status names from your database
      { NEW: 0, PENDING: 0, DONE: 0 }
    );
  }, [accidents]);

  // This hook now filters correctly by comparing case-insensitively
  ...
  // Data for the stat cards, now using the correct uppercase keys
  const statCardsData = [
